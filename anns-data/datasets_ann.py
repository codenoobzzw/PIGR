...
#from datasets import DATASETS, get_dataset_fn
...
def write_output(train: numpy.ndarray, test: numpy.ndarray, fn: str, distance: str, point_type: str = "float", count: int = 100) -> None:
    ...
    with h5py.File(fn, "w") as f:
        ...
        from binary import ivecs_save, fvecs_save

        fp = fn.split(".")[0]

        if train.dtype == numpy.float32 or train.dtype == numpy.float64:
            fvecs_save(fp + ".train.fvecs", train)
        elif train.dtype == numpy.int32:
            ivecs_save(fp + ".train.ivecs", train)
        else:
            raise ValueError("Unsupported data type")
        
        if test.dtype == numpy.float32 or test.dtype == numpy.float64:
            fvecs_save(fp + ".test.fvecs", test)
        elif test.dtype == numpy.int32:
            ivecs_save(fp + ".test.ivecs", test)
        else:
            raise ValueError("Unsupported data type")
...
def glove(out_fn: str, d: int) -> None:
    ...
    with zipfile.ZipFile(fn) as z:
        ...
        write_output(numpy.array(X_train), numpy.array(X_test), out_fn, "euclidean", "float")
...
def audio(out_fn: str) -> None:
    import tarfile

    url = "https://www.cse.cuhk.edu.hk/systems/hash/gqr/dataset/audio.tar.gz"
    fn = os.path.join("data", "audio.tar.gz")
    download(url, fn)

    # 假设压缩包里有 audio_base.fvecs 和 audio_query.fvecs
    with tarfile.open(fn, "r:gz") as t:
        # 提取训练集
        train_member = None
        test_member = None
        for member in t.getmembers():
            if member.name.endswith("audio_base.fvecs"):
                train_member = member
            elif member.name.endswith("audio_query.fvecs"):
                test_member = member

        if train_member is None or test_member is None:
            raise FileNotFoundError("audio_base.fvecs 或 audio_query.fvecs 未找到")

        t.extract(train_member, path="data")
        t.extract(test_member, path="data")
        train_path = os.path.join("data", train_member.name)
        test_path = os.path.join("data", test_member.name)

    # 读取 fvecs 格式
    train = numpy.fromfile(train_path, dtype=numpy.float32)
    dim = train.view(numpy.int32)[0]
    train = train.reshape(-1, dim + 1)[:, 1:]

    test = numpy.fromfile(test_path, dtype=numpy.float32)
    test = test.reshape(-1, dim + 1)[:, 1:]
...
def _load_mnist_vectors(fn: str) -> numpy.ndarray:
    ...
    entry_size = numpy.prod(dimensions[1:])  # 修复：改为 numpy.prod
...
def deep_image(out_fn: str) -> None:
    ...
    url = "https://www.cse.cuhk.edu.hk/systems/hash/gqr/dataset/deep1M.tar.gz"
    fn = os.path.join("data", "deep1M.tar.gz")
    download(url, fn)
    ...
    with tarfile.open(fn, "r:gz") as t:
        ...
        test_member = None
        for member in t.getmembers():
            if member.name.endswith("deep1M_base.fvecs"):
                train_member = member
            elif member.name.endswith("deep1M_query.fvecs"):
                test_member = member

        if train_member is None or test_member is None:
            raise FileNotFoundError("deep1M_base.fvecs 或 deep1M_query.fvecs 未找到")
...
def transform_bag_of_words(filename: str, n_dimensions: int, out_fn: str, distance: str = "angular") -> None:
    import gzip
    ...
    with gzip.open(filename, "rb") as f:
        ...
        write_output(numpy.array(X_train), numpy.array(X_test), out_fn, distance)


def nytimes(out_fn: str, n_dimensions: int, distance: str = "angular") -> None:
    ...
    transform_bag_of_words(fn, n_dimensions, out_fn, distance)
...
def kosarak(out_fn: str) -> None:
    ...
    raise NotImplementedError("Cannot convert HDF5 to vecs-format currently.")
...
def dbpedia_entities_openai_1M(out_fn, n = None):
    ...
    print(len(data))  # 应该是1000000
    print(data.column_names)  # 检查字段
    print(data[0])  # 查看第一条数据内容
    ...
    write_output(X_train, X_test, out_fn, "euclidean", "float")

DATASETS: Dict[str, Callable[[str], None]] = {
    "deep-image-96-euclidean": deep_image,
    "fashion-mnist-784-euclidean": fashion_mnist,
    "gist-960-euclidean": gist,
    "glove-25-angular": lambda out_fn: glove(out_fn, 25),
    "glove-50-angular": lambda out_fn: glove(out_fn, 50),
    "glove-100-eulidean": lambda out_fn: glove(out_fn, 100),
    ...
    "nytimes-256-angular": lambda out_fn: nytimes(out_fn, 256, "angular"),
    "nytimes-16-angular": lambda out_fn: nytimes(out_fn, 16, "angular"),
    "nytimes-256-euclidean": lambda out_fn: nytimes(out_fn, 256, "euclidean"),
    "nytimes-16-euclidean": lambda out_fn: nytimes(out_fn, 16, "euclidean"),
    ...
    "audio-192-euclidean": audio,
}

DATASETS.update({
    f"dbpedia-openai-{n//1000}k-euclidean": lambda out_fn, i=n: dbpedia_entities_openai_1M(out_fn, i)
